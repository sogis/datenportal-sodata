...
class ExplorePageControllerMvcTest {
    ...
    void explorePageRendersPortalHostAndEmbeddedContext() throws Exception {
        mockMvc.perform(get("/datasets/ch.so.bauinventar/explore"))
                ...
                .andExpect(content().string(containsString("\"chartsEnabled\":true")))
                .andExpect(content().string(containsString("\"webREnabled\":true")))
                .andExpect(content().string(containsString("\"rLaboratory\"")))
                .andExpect(content().string(containsString("\"dataFrameName\":\"daten\"")))
                .andExpect(content().string(not(containsString("aiAssistant"))))
    ...
    void contextJsonReturnsExpectedShape() throws Exception {
        mockMvc.perform(get("/datasets/ch.so.bauinventar/explore/context.json"))
                ...
                .andExpect(jsonPath("$.catalogDatabase.url").value(
                        "/catalog/catalog.duckdb?v=2cbe0775a353408e007a425a701ef66ed8a37c5b7bf421db0b08c7c471667fd3"))
