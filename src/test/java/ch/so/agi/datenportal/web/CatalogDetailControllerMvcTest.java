...
class CatalogDetailControllerMvcTest {
    ...
    void datasetDetailRendersTitleDescriptionDownloadsMetadataAndBreadcrumb() throws Exception {
        mockMvc.perform(get("/datasets/ch.so.bauinventar"))
                ...
                .andExpect(content().string(containsString("Aufbau, Herkunft und Prüfergebnisse der Daten nachvollziehen.")))
                .andExpect(content().string(containsString("Mehr über die Daten erfahren <span aria-hidden=\"true\">&rarr;</span>")))
                .andExpect(content().string(containsString("aria-label=\"Mehr über die Daten erfahren: Struktur, Qualität und Herkunft\"")))
                .andExpect(content().string(containsString("Daten direkt im Browser abfragen, auswerten und visualisieren.")))
                .andExpect(content().string(containsString("Daten erkunden <span aria-hidden=\"true\">&rarr;</span>")))
                .andExpect(content().string(containsString("aria-label=\"Daten erkunden: Daten im Browser abfragen, auswerten und visualisieren\"")))
                .andExpect(content().string(containsString("Daten herunterladen und mit Codebeispielen in eigene Anwendungen einbinden.")))
                .andExpect(content().string(containsString("Downloads und Codebeispiele anzeigen <span aria-hidden=\"true\">&rarr;</span>")))
                .andExpect(content().string(containsString("aria-label=\"Downloads und Codebeispiele anzeigen: Daten verwenden\"")))
    ...
    void currentIssueDetailRendersConcreteIssueWithDatasetDetailCards() throws Exception {
        mockMvc.perform(get("/series/ch.so.abstimmungsresultate/issues/current"))
                ...
                .andExpect(content().string(not(containsString("Daten erkunden nicht verfügbar"))))
                .andExpect(content().string(not(containsString("href=\"#\""))))
                .andExpect(content().string(containsString("Daten erkunden <span aria-hidden=\"true\">&rarr;</span>")))
                .andExpect(content().string(containsString("<aside class=\"dp-detail-side\" aria-label=\"Daten nutzen\">")))
    ...
    void nonOpenDatasetDetailShowsAccessBadgeAndLockInsteadOfDownloads() throws Exception {
        mockMvc.perform(get("/datasets/ch.2581.baumkataster"))
                ...
                .andExpect(content().string(containsString("Daten erkunden nicht verfügbar")))
                .andExpect(content().string(containsString("Downloads und Codebeispiele nicht verfügbar")))
                ...
                .andExpect(content().string(not(containsString("href=\"#\""))))
                .andExpect(content().string(not(containsString("Daten erkunden <span aria-hidden=\"true\">&rarr;</span>"))))
                .andExpect(content().string(not(containsString("Downloads und Codebeispiele anzeigen <span aria-hidden=\"true\">&rarr;</span>"))))
