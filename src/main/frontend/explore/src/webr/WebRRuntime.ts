...
export class WebRRuntime {
  ...
  initialize(): Promise<WebRLike> {
    this.webRPromise ??= this.load().catch((error: unknown) => {
  ...
  close(): void {
    void this.webRPromise?.then((webR) => webR.close()).catch(() => undefined);
  }
  ...
  private async load(): Promise<WebRLike> {
    ...
    await withTimeout(webR.init(), 120_000, 'Die WebR-Runtime konnte nicht initialisiert werden.');
