...
- `check` haengt von `playwrightTest` ab und installiert Chromium und Firefox bei Bedarf ueber `installPlaywrightChromium` und `installPlaywrightFirefox`.
...
- Seit der Exportsteuerungs-Verschiebung pruefen Vitest und Playwright, dass der Tabellen-Splitbutton im Ergebnis-Header statt im SQL-Toolbar-Bereich sitzt, im Diagrammmodus nur der PNG-Button erscheint und dieser bei einem nicht renderbaren Diagramm deaktiviert bleibt. Der PNG-Unit-Test prueft Dateiname, Titel-/Legendeninhalt, das Entfernen markierter UI-Elemente, die aktivierte Option `skipFonts` und die DOM-Aufräumung bei Erfolg und Fehler.
- Der PNG-Regressionstest verwendet den Serien-Explore-Pfad. Der Chromium-Test validiert neben Signatur und `ImageIO` auch echte Diagrammfarben und ausreichende Farbvielfalt, damit ein formal gültiges, aber nur aus periodischem Canvas-Rauschen bestehendes PNG erkannt wird. Ein zusätzlicher Firefox-Test startet mit aktiviertem `privacy.resistFingerprinting`, erwartet die verständliche Ergebniswarnung und stellt sicher, dass kein Download beginnt. Beide Tests verbieten die früheren `html-to-image`-Fehlermeldungen für remote CSS und den Font-Inliner.
- Der Frontend-Unit-Test prüft den Canvas-Vorabtest: unveränderte Vollton-Pixel erlauben `toPng`, geschützte oder verrauschte Pixel lösen den Exportfehler vor dem Download aus. Die bestehende Prüfung von Dateiname, Titel-/Legendeninhalt, markierten UI-Elementen, `skipFonts` und Clone-Aufräumung bleibt bestehen.
...
- Seit der 64-bit-Typisierung pruefen Vitest-Tests das wertbasierte Mapping: BIGINT-Werte im int32-Bereich werden `integer`, Werte bis 2^53 − 1 `numeric`, groessere `character`, Identifier bleiben `character`, und der Snapshot uebertraegt die Werte als Zahlen statt als Strings.
- Seit dem R-Rezept-Ausbau pruefen Vitest- und Java-Tests die wertebasierte Jahrserkennung (`Jahrgang`), Balken-, Streu- und Mehrfachtrend-Rezepte, den Anzahl-Fallback ohne Messwert und dass Zeitspalten nie als Messwert dienen.
- Seit der R-Plot-Farbangleichung prueft Vitest, dass einfarbige R-Rezepte Dunkelblau `#104E8B` verwenden, Linie und Punkte im Trend dieselbe Farbe haben und der Mehrfachtrend die ersten Farben der `Mehrfarbig`-Palette setzt.
- Der R-Datenbasis-Panel-Test prueft die einheitliche Titelhierarchie mit `DATENGRUNDLAGE` als einzigem Seitentitel und stellt sicher, dass `DATA FRAME` weiterhin als Faktenlabel statt als grosse Ueberschrift gerendert wird.
