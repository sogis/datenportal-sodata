...
- `YEAR`: Spaltennamen `year`, `periode`, `jahrgang` oder Namen mit Endung `jahr` (`jahr`, `berichtsjahr`, `baujahr`, `erfassungsjahr`).
...
Mit Snapshot entstehen immer `Datenüberblick`, `Spaltenstruktur` und `Fehlende Werte`. Danach waehlt die Logik bis zu zwei Datum-/Jahrspalten, bis zu drei numerische Messwertspalten und bis zu zwei Kategorie-Spalten. Die Auswahl nutzt dieselben wertebasierten Helfer wie `chartInference`: Werte im Bereich 1800–2200 mit mindestens 70 Prozent Anteil gelten als Jahr, ISO-Werte oder `Date`-Instanzen als Datum. Dadurch werden auch Spalten wie `Jahrgang` als Zeitachse erkannt, obwohl ihr Name nicht exakt `jahr` lautet. Die numerische Auswahl bevorzugt `numeric`, die Rolle `measure` und Namen wie `messwert`, `wert`, `quote`, `rate` oder `index`; sie bestraft Jahr-/Monat-/Tag-Spalten sowie IDs, Nummern und Codes. Zeitspalten und Identifier werden nie als Messwert verwendet. Die Kategorie-Auswahl bevorzugt Rollen wie `category`, `municipality` und `label` sowie Namen wie `gemeinde`, `name`, `parameter`, `status` oder `thema`; Codes, IDs, Nummern und Zeitspalten werden aussortiert.

Aus diesen Spalten entstehen optional `Numerische Zusammenfassung`, `Histogramm «<messwert>»`, `Balkendiagramm «<kategorie>»`, `Mittelwert «<messwert>» nach «<kategorie>»`, `Boxplot «<messwert>» nach «<kategorie>»`, `Trend «<messwert>» nach «<zeit>»`, `Trend «<messwert>» + «<messwert>» nach «<zeit>»`, `Streudiagramm «<messwert>» und «<messwert>»` und als Fallback `Anzahl pro «<zeit>»`, wenn eine Zeitspalte ohne Messwert vorliegt. Histogramm, Boxplot, Streudiagramm und der Anzahl-Fallback plotten Rohdaten und setzen deshalb im generierten R-Code `plot_limit <- 10000`; Trend-, Mehrfachtrend- und Mittelwert-Balken-Rezepte aggregieren zuerst. Die Rezepttitel verwenden Schweizer Anfuehrungszeichen, damit Spaltennamen im Dropdown gleich markiert sind wie im SQL-Labor.

Die generierten Plots verwenden dieselben Diagrammfarben wie das SQL-Labor. Einfarbige Rezepte zeichnen in Dunkelblau (`#104E8B`); Linien und Punkte eines Trends sind beide Dunkelblau. Der Mehrfachtrend nutzt fuer mehrere Reihen die ersten Farben der `Mehrfarbig`-Palette aus `chartColors.ts`, damit R-Code und SQL-Diagramme dieselbe Farbdefinition teilen.
...
- Linien- und Balkendiagramme nutzen `SeriesTooltipContent` als Tooltip-Inhalt: Die Kopfzeile zeigt den echten X-Wert (Jahr oder Kategorie), die Reihenwerte werden gemäss `de-CH` formatiert. Damit wird vermieden, dass die Bibliothek bei numerischer X-Achse das erste Reihenlabel als Überschrift wiederholt und Werte in der Browser-Locale darstellt (`1,564` statt `1’564`). Null-Werte bleiben ausgeblendet, `0` wird angezeigt.
...
Das R-Labor kann WebR auch ohne SQL-Result initialisieren. Ein fachlicher Dataframe entsteht aber nur durch ein explizit aus dem SQL-Labor uebernommenes Resultat. Aus `QueryResultState.arrowTable` entsteht dann ein typisierter `SqlResultSnapshot` mit Spaltennamen, DuckDB-/Arrow-nahen Typen, Nullable-Info, Rollen und Zeilen. Das Mapping ist verlustfrei und wertbasiert: IDs, `DECIMAL`, Geometrien und Binaries bleiben `character`; 64-bit Integer werden anhand der tatsaechlichen Werte des Resultats als `integer` (innerhalb des int32-Bereichs) oder `numeric` (bis 2^53 − 1 exakt in double) uebernommen und nur bei groesseren oder nicht exakt lesbaren Werten verlustfrei als `character` gehalten; 32-bit Integer werden `integer`; Float/Double werden `numeric`; `DATE` wird `Date`; Timestamps werden `POSIXct` in UTC. Die Entscheidung gilt pro Resultat-Snapshot: Liefert dieselbe Spalte spaeter groessere Werte, kann der R-Typ auf `numeric` oder `character` wechseln. Der Transfer erfolgt als JSON-Snapshot plus R-Konvertierungsskript. In WebR stehen danach `daten`, `daten_schema` und `attr(daten, "duckdb_schema_json")` zur Verfuegung.
...
Linie und Balken teilen sich `SeriesTooltipContent`, damit die Tooltip-Kopfzeile
den X-Wert zeigt und Zahlen in `de-CH` erscheinen.
