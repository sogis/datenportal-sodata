...
Status: SQL- und R-Labor with catalog-backed Schema Explorer implemented
...
- Resultate koennen im Ergebnis-Header als CSV, XLSX und Parquet exportiert werden; exportiert wird nur das aktuell gelieferte Query-Resultat. CSV bleibt wegen Semikolon/CRLF clientseitig, XLSX und Parquet werden per DuckDB-Wasm `COPY` erzeugt. Im Diagrammmodus wird das sichtbare Diagramm mit Titel und Pie-/Donut-Legende als PNG exportiert; Steuerfelder und technische Hinweise werden dabei ausgelassen. Der PNG-Export bettet keine externen Fonts ein, damit relative Font-URLs in Browsern wie Firefox nicht den Export blockieren.
